...
| Licences: open-design, impeccable, Qwen3.8 (fine-tuning). | Check before M4.2 and M7.1 respectively. |
| The Sandpack bundler build is GPLv3 (M0.3). | Self-hosted, unmodified, on its own origin, with its source published (ADR-0002 Decision 5). |
| The self-hosted bundler still fetches npm packages from CodeSandbox's packager. | Evaluate an npm packager/registry proxy before production (ADR-0002 open question 3). |
